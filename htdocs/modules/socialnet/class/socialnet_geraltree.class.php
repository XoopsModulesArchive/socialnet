...
if (!class_exists('socialnet_geral')) {
    class socialnet_geral extends XoopsObject
    {
        ...
        function administration($url, $fields) {
            ...
            if(!empty($fields['precrit']['field']) && !empty($fields['precrit']['valor'])){
                ...
                foreach ($fields['precrit']['field'] as $k=>$v) {
                    $operador = (isset($fields['precrit']['operador'][$k])) ? $fields['precrit']['operador'][$k] : "=";
                    $hiddens[$v] = $fields['precrit']['valor'][$k];
                    $criterio->add(new Criteria($v, $fields['precrit']['valor'][$k], $operador, $this->tabela));
		...
		tr.hd td {background-image:url("../images/icons/bg.gif"); padding: 5px; font-weight: bold; border:1px solid #C0C0C0; color: #000000}
		tr.hd td.hds {background-image:url("../images/icons/bgs.gif"); padding: 5px; font-weight: bolder; border:1px solid #C0C0C0; border-top: 1px solid #000000; color: #000000}
...
<tr><td class="outer" style="background-color: #F3F2F2;"><div style="text-align: center;">';
            ...
            foreach ($fields['label'] as $k => $v) {
                $ret .= "<td nowrap='nowrap' align='center' ".(($sort == $fields['name'][$k] && empty($fields['nosort'][$k])) ? "class='hds'" : '').">".((empty($fields['nosort'][$k])) ? "<A HREF='".$url_colunas."&sort=".$fields['name'][$k]."&order=".$norder."'>".$v." ".(($sort == $fields['name'][$k]) ? "<img src='../images/icons/".$order.".gif' align='absmiddle'>" : '')."</a></td>" : $v."</td>");
            }
