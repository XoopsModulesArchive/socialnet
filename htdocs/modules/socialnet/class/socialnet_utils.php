...
class socialnet_utils
{

	const MODULE_NAME = 'socialnet';
	 ...
	 * @param integer timeout Time to wait before to redirect
 	 */
	function redirect($message='', $url='pageuser.php', $time=2)
	{
	...
	protected function _getModule()
	{
		...
		if (!isset($mymodule)) {
			global $xoopsModule;
			if (isset($xoopsModule) && is_object($xoopsModule) && $xoopsModule->getVar('dirname') == SOCIALNET_DIRNAME ) {
				$mymodule =& $xoopsModule;
			} else {
				$hModule = &xoops_gethandler('module');
				$mymodule = $hModule->getByDirname(SOCIALNET_DIRNAME);
			}
		}
		return $mymodule;
	}
