...
class SocialnetTopic extends XoopsTopic
{
    var $banner = "";
    var $banner_inherit;
    var $forum_id;
	...
	function SocialnetTopic($table, $topicid=0)
	{
	...
	function store()
	{
		...
		//Added in Socialnet  Fix bug permission not set at add topic
		//if ADD TOPIC
		if($insert) {
	...
	function getBanner() {
        ...
        $parent_topic = new SocialnetTopic($this->table, $this->topic_pid());
        return $parent_topic->getBanner();
	}
	
	function getAllTopics($checkRight = false, $permission = "socialnet_view") {
	    static $topics_arr = array();
	    $db =& Database::getInstance();
	    $table = $db->prefix('socialnet_topics');
	    if ((!isset($topics_arr['checked']) && false != $checkRight) || (!isset($topics_arr['unchecked']) && false == $checkRight)){
	        ...
	        while ($array = $db->fetchArray($result)) {
	            $topic = new SocialnetTopic($table);
	...
	function getAllAuthors($byName = false) {
	    ...
	    $db =& Database::getInstance();
	    if (count($authors) == 0) {
	        $sql = "SELECT DISTINCT u.uid, u.uname, u.name FROM ".$db->prefix('users')." u, ".$db->prefix('socialnet_text')." t
	            WHERE u.uid = t.uid";
	        if (false != $byName) {
	...
	function getTopicPath($withAllLink = false)
	{
        $filename = "news.php";
		If ($withAllLink) {
			$ret = "<a href='" . XOOPS_URL . "/modules/socialnet/".$filename."?storytopic=" . $this->topic_id() . "'>" . $this->topic_title() . "</a>";	
		} else {
		...
		if ($parentid > 0) {
		    ...
		    $ret = "<a href='".XOOPS_URL."/modules/socialnet/".$filename."'>".$GLOBALS['xoopsModuleConfig']['index_name']."</a> > ".$ret;
		}
		return $ret;
	}
