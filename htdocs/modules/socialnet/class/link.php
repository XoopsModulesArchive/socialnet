...
class Link extends XoopsObject {
    ...
    function Link($id = null) {
        ...
        $this->table = $this->db->prefix('socialnet_newslink');
        if ($id != null) {
            $this->get($id);
        }
    }
...
class socialnetLinkHandler extends IdgObjectHandler {
    function socialnetLinkHandler(&$db) {
        $this->IdgObjectHandler($db, 'socialnet_newslink', 'Link', 'linkid');
    }
    ...
    */
    function &getByStory($storyid) {
	    ...
	    $link = "newsarticle.php?storyid=".intval($storyid);
	    $myts = MyTextSanitizer::getInstance();
	    
	    if ($xoopsModule->getVar('dirname') != "socialnet") {
	        $newsmodule =& $module_handler->getByDirname('socialnet');
	    }
	    else {
	        $newsmodule =& $xoopsModule;
	    }
	    $sql = "SELECT n.title, n.storyid, l.* FROM ".$this->table." l, ".$this->db->prefix('socialnet_article')." n WHERE n.storyid=l.storyid AND ((link_link='$link' AND link_module=".$newsmodule->mid().") OR (l.storyid = ".intval($storyid)."))";
	    ...
	    while ($row = $this->db->fetchArray($directresult)) {
	        if ($row['storyid'] == $storyid) {
	            ...
	            $row['link_link'] = 'newsarticle.php?storyid='.$row['storyid'];
	    ...
	    foreach ($ret as $position => $links) {
	        foreach ($links as $key => $link) {
	            $ret[$position][$key]['link_module'] = ($link['link_module'] > -1) ? $modules[$link['link_module']] : _MD_SOCIAL_NW_EXTERNALLINK;
	        }
	    }
	    return $ret;
    }
