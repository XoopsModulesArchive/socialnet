...
class SocialnetStory extends XoopsStory
{
    ...
    function SocialnetStory($storyid=-1, $getRating = false)
    {
        $this->db =& Database::getInstance();
        $this->table = $this->db->prefix("socialnet_article");
        $this->texttable = $this->db->prefix("socialnet_text");
        $this->topicstable = $this->db->prefix("socialnet_topics");
        $this->audiencetable = $this->db->prefix("socialnet_newsaudience");
        if (is_array($storyid)) {
    ...
    function topic($single_topic = false)
    {
        if (!$this->newstopic) {
            if (false == $single_topic) {
                ...
                $this->newstopic = new SocialnetTopic($this->topicstable, $this->topicid());
            }
        }
    ...
    function getAllPublished($limit=0, $start=0, $checkRight = false, $topic=0, $ihome=0, $asobject=true, $order = 'published', $ids = false)
    {
        ...
        $myts =& MyTextSanitizer::getInstance();
        $ret = array();
        $sql = "SELECT * FROM ".$db->prefix("socialnet_article")." n, ".$db->prefix("socialnet_text")." t, ".$db->prefix("socialnet_newsaudience")." a WHERE t.storyid=n.storyid AND n.audienceid=a.audienceid AND published > 0 AND published <= ".time()." AND (expired = 0 OR expired > ".time().")";
        if ( $topic != 0 ) {
            if (!is_array($topic)) {
            ...
            if ($checkRight) {
                ...
                if (!isset($xoopsModule) || $xoopsModule->getVar('dirname') != "socialnet") {
                    $module_handler =& xoops_gethandler('module');
                    $newsModule =& $module_handler->getByDirname('socialnet');
                }
                else {
                ...
                $topics = $gperm_handler->getItemIds('socialnet_view', $groups, $newsModule->getVar('mid'));
    ...
    */
    function getAllSubmitted($limit=0, $asobject=true, $checkRight = false)
    {
        ...
        if ($checkRight) {
            ...
            $allowedtopics = $gperm_handler->getItemIds('socialnet_approve', $groups, $newsmodule->getVar('mid'));
        ...
        $sql = "SELECT * FROM ".$db->prefix("socialnet_article")." n, ".$db->prefix("socialnet_text")." t WHERE n.storyid=t.storyid AND t.current=1";
    ...
    function getByTopic($topicid, $limit=0)
    {
        $ret = array();
        $db =& Database::getInstance();
        $sql = "SELECT * FROM ".$db->prefix("socialnet_article")." n, ".$db->prefix("socialnet_text")." t, ".$db->prefix("socialnet_newsaudience")." a WHERE n.storyid=t.storyid AND n.audienceid=a.audienceid AND t.current=1 AND topicid=".intval($topicid)." AND published > 0 AND published <= ".time()." AND (expired = 0 OR expired > ".time().") ORDER BY published DESC";
    ...
    {
        $db =& Database::getInstance();
        $sql = "SELECT COUNT(*) FROM ".$db->prefix("socialnet_article")."
		WHERE expired >= ".time()."";
        if ( $topicid != 0 ) {
    ...
    {
        $db =& Database::getInstance();
        $sql = "SELECT COUNT(*) FROM ".$db->prefix("socialnet_article")." WHERE published > 0 AND published <= ".time()." AND (expired = 0 OR expired > ".time().")";
        if ( !empty($topicid) ) {
            $sql .= " AND topicid=".intval($topicid);
        } else {
            $sql .= " AND ihome=0";
    ...
    {
        $db =& Database::getInstance();
        $sql = "SELECT topicid, COUNT(*) FROM ".$db->prefix("socialnet_article")." WHERE published > 0 AND published <= ".time()." AND (expired = 0 OR expired > ".time().")";
        if ( !empty($topicid) ) {
            $sql .= " AND topicid=".intval($topicid);
        } else {
            $sql .= " AND ihome=0";
    ...
    function countReads() {
        ...
        $sql = "SELECT SUM(counter) FROM ".$db->prefix('socialnet_article')." WHERE published > 0 AND published <= ".time()." AND (expired = 0 OR expired > ".time().")";
        $result = $db->query($sql);
        list($count) = $db->fetchRow($result);
        return $count;
    }
    ...
    function imglink($show_avatar = false, $users = false)
    {
        ...
        if ($this->newstopic->topic_imgurl() != '' && file_exists(XOOPS_ROOT_PATH."/modules/socialnet/images/topics/".$this->newstopic->topic_imgurl())) {
            $ret = "<a href='".XOOPS_URL."/modules/socialnet/news.php?storytopic=".$this->topicid."'><img src='".XOOPS_URL."/modules/socialnet/images/topics/".$this->newstopic->topic_imgurl()."' alt='".$this->newstopic->topic_title()."' hspace='10' vspace='10' align='".$this->topicalign()."' /></a>";
        }
        return $ret;
    }
    ...
    function store()	{
        ...
        if ( !isset($this->storyid) ) {
            ...
            if (!$this->db->query($sql)) {
                $this->_errors[] = _MD_SOCIAL_NW_SAVEFAILED;
                return false;
            }
    ...
    function setCurrentVersion($version, $revision, $revisionminor) {
        ...
        if (!$this->db->query($sql)) {
            $this->_errors[] = _MD_SOCIAL_NW_COULDNOTRESET;
            return false;
        }
        ...
        if (!$this->db->query($sql)) {
            $this->_errors[] = _MD_SOCIAL_NW_COULDNOTUPDATEVERSION;
            return false;
        }
        return true;
    }
    ...
    function updateVersion() {
        ...
        if (!$this->db->query($sql)) {
            $this->_errors[] = _MD_SOCIAL_NW_TEXTSAVEFAILED;			
            return false;
        }
        if (!$this->setCurrentVersion($this->version, $this->revision, $this->revisionminor)) {
            $this->_errors[] = _MD_SOCIAL_NW_VERSIONUPDATEFAILED;
            return false;
        }
        return true;
    }
    ...
    function getRatings() {
        $sql = "SELECT * FROM ".$this->db->prefix("socialnet_newsrating")." WHERE storyid = ".$this->storyid;
    ...
    function rateStory($rating) {
        ...
        if(!$xoopsUser)
        {
            if ($xoopsModuleConfig['anonymous_vote'] == 0) {
                $this->_errors[] = _MD_SOCIAL_NW_ANONYMOUSVOTEDISABLED;
                return false;
            }
            $ratinguser = 0;
            if (count($this->ratings) > 0) {
                foreach ($this->ratings as $key => $thisrating) {
                    if ($ip == $thisrating['ratinghostname']) {
                        $this->_errors[] = _MD_SOCIAL_NW_ANONYMOUSHASVOTED;
                        return false;
                    }
                }
            }
        ...
        else
        {
            ...
            if ($ratinguser == $this->uid()) {
                $this->_errors[] = _MD_SOCIAL_NW_CANNOTVOTESELF;
                return false;
            }
            foreach ($this->ratings as $key => $thisrating) {
                if ($ratinguser == $thisrating['ratinguser']) {
                    $this->_errors[] = _MD_SOCIAL_NW_USERHASVOTED;
                    return false;
                }
            }
        }
    ...
    function saveRating($rating, $uid, $hostname) {
        $rating = intval($rating);
        $uid = intval($uid);
        $sql = "INSERT INTO ".$this->db->prefix("socialnet_newsrating")." (storyid, ratinguser, rating, ratinghostname, ratingtimestamp) VALUES (".$this->storyid.", $uid, $rating, ".$this->db->quoteString($hostname).", ".time().")";
        if ($this->db->query($sql)) {
        ...
        $this->_errors[] = _MD_SOCIAL_NW_COULDNOTSAVERATING;
        $this->_errors[] = $sql;
        return false;
    }
    ...
    function updateAvgRating() {
        ...
        if (!$this->db->query($sql)) {
            $this->_errors[] = _MD_SOCIAL_NW_COULDNOTUPDATERATING;
            $this->_errors[] = $sql;
            return false;
        }
        return true;
    ...
    function addLink($moduleid, $link, $title, $position) {
        $linkHandler =& xoops_getmodulehandler('link', 'socialnet');
        ...
        if (!$linkHandler->insert($thisLink)) {
            $this->_errors[] = _MD_SOCIAL_NW_COULDNOTADDLINK;
            return false;
        }
        return true;
    }
    ...
    function deleteLink($linkid) {
        ...
        if (!$linkHandler->delete($link)) {
            $this->_errors[] = _MD_SOCIAL_NW_COULDNOTDELLINK;
            return false;
        }
        return true;
    }
    ...
    function renderErrors() {
        if (count($this->_errors) == 0) {
            return _MD_SOCIAL_NW_NOERRORSENCOUNTERED;
        }
        $ret = "";
    ...
    function toArray($admin = false, $pagenav = false, $storypage = 0, $users = false) {
        ...
        if ( trim($bodytext) != '' ) {
            if (false != $pagenav) {
                ...
                if ($story_pages > 1 && $storypage != -1) {
                    ...
                    $story['bodytext'] = implode('<br /><b>'._MD_SOCIAL_NW_PAGEBREAK."</b><br />", $articletext);
                }
                else {
        ...
        $story['bytestext'] = _MD_SOCIAL_NW_READMORE;

        $ccount = $this->comments();
        if ($ccount == 0) {
            $story['comments'] = _MD_SOCIAL_NW_COMMENTS;
        }
        elseif ($ccount == 1) {
            $story['comments'] = _MD_SOCIAL_NW_ONECOMMENT;
        }
        else {
            $story['comments'] = sprintf(_MD_SOCIAL_NW_NUMCOMMENTS, $ccount);
        }
        ...
        $story['ratingimage'] = XOOPS_URL."/modules/socialnet/images/icons/rate".$this->getRating().".png";
        $story['rating'] = $this->getRating();
        $story['hits'] = $this->counter();
        $story['mail_link'] = 'mailto:?subject='.sprintf(_MD_SOCIAL_NW_INTARTICLE,$xoopsConfig['sitename']).'&amp;body='.sprintf(_MD_SOCIAL_NW_INTARTFOUND, $xoopsConfig['sitename']).':  '.XOOPS_URL.'/modules/socialnet/newsarticle.php?storyid='.$this->storyid();
    ...
    function delversions($version, $revision, $minor) {
        ...
        $revision = intval($revision);
        $minor = intval($minor);
        $sql = "DELETE FROM ".$this->db->prefix('socialnet_text')." WHERE storyid=".intval($this->storyid)."
	            AND (
	           (version < $version) OR 
    ...
    function delallversions($version, $revision, $minor) {
        ...
        $sql = "DELETE FROM ".$this->db->prefix('socialnet_text')." WHERE storyid=".intval($this->storyid)." AND current=0";
        if ($this->db->query($sql)) {
            return true;
        }
        return false;
    ...
    function getNewsVersion($storyid, $version, $revision, $revisionminor = 0) {
        $sql = "SELECT * FROM ".$this->db->prefix("socialnet_article")." n, ".$this->db->prefix("socialnet_text")." t
    ...
    */
    function getAllNews($limit, $start = 0, $criteria = null) {
        ...
        $sql = "SELECT n.*, t.*, a.* FROM ".$db->prefix('socialnet_article')." n, ".$db->prefix('socialnet_text')." t, ".$db->prefix('socialnet_newsaudience')." a WHERE n.storyid=t.storyid AND n.audienceid=a.audienceid AND n.published != 0 AND t.current=1";
    ...
    */
    function getVersionCounts($storyids = array()) {
        ...
        $ret = array();
        $db =& XoopsDatabaseFactory::getDatabaseConnection();
        $sql = "SELECT storyid, count(version) AS VersionCount FROM ".$db->prefix('socialnet_text')." WHERE storyid IN (".implode(',', $storyids).") GROUP BY storyid";
        $result = $db->query($sql);
    ...
    */
    function getAuthors($limit = 5, $sort = "count", $name = 'uname', $compute_method = "average") {
        ...
        if ($sort == "count") {
            $sql = "SELECT u.".$name." AS name, u.uid , count( n.storyid ) AS count
	                FROM ".$db->prefix("users")." u, ".$db->prefix("socialnet_article")." n, ".$db->prefix("socialnet_text")." t
    ...
    function getPath($withAllLink = false) {
        if (!$withAllLink) {
            ...
            return $this->newstopic->getTopicPath(true)." > <a href='newsarticle.php?storyid=".$this->storyid."'>".$this->title()."</a>";
        }
    }
}
?>