...
class socialnet_mpb_mpublish extends socialnet_geral {
    function socialnet_mpb_mpublish($id=null){
        $this->db =& Database::getInstance();
        $this->tabela = $this->db->prefix(_MI_SOCIALNET_TABLEPUBLISH);
        $this->id = "mpb_10_id";
        ...
        $this->initVar("mpb_30_title", XOBJ_DTYPE_TXTBOX, null, false);
        $this->initVar("mpb_35_content", XOBJ_DTYPE_TXTAREA, null, false);
        $this->initVar("mpb_12_withoutlink", XOBJ_DTYPE_INT, 0, false);
        $this->initVar("mpb_30_file", XOBJ_DTYPE_TXTBOX, null, false);
        $this->initVar("mpb_11_visible", XOBJ_DTYPE_INT, 2, false);
        $this->initVar("mpb_11_open", XOBJ_DTYPE_INT, 0, false);
        $this->initVar("mpb_12_comments", XOBJ_DTYPE_INT, null, false);
        $this->initVar("mpb_12_exibesub", XOBJ_DTYPE_INT, 1, false);
        $this->initVar("mpb_12_recommend", XOBJ_DTYPE_INT, 1, false);
        $this->initVar("mpb_12_print", XOBJ_DTYPE_INT, 1, false);
        $this->initVar("mpb_22_create", XOBJ_DTYPE_INT, 0, false);
        $this->initVar("mpb_22_uptodate", XOBJ_DTYPE_INT, 0, false);
        $this->initVar("mpb_10_order", XOBJ_DTYPE_INT, 0, false);
        $this->initVar("mpb_10_counter", XOBJ_DTYPE_INT, 0, false);
        if ( !empty($id) ) {
            if ( is_array($id) ) {
                $this->assignVars($id);
    ...
    function tem_subcategorias($mpb_10_id=null, $menu = false) {
        ...
        if(!$menu){
            ...
            $pages = $this->getPages();
            $cat_filha_query = $this->db->query("SELECT ".$this->id." FROM " . $this->tabela . " where mpb_10_idpai =" . $id . " AND mpb_11_visible < 3");
    ...
    function geraMenuSelect($mpb_10_idpai=0, $modules = true) {
        ...
        $tem_subs = 0;
        $cat_principal = 0;
        $categorias_query_catmenu = $this->db->query("select mpb_10_id, mpb_10_idpai, mpb_30_menu from ".$this->tabela." where mpb_10_idpai=" . (int)$mpb_10_idpai . " order by mpb_10_order, mpb_30_menu");
    ...
    function geraMenuCSS($criterio = null, $modulos = true) {
        ...
        $criterio->setSort("mpb_10_order");
        $criterio->add(new Criteria("mpb_11_visible", 3, "<"));
        $module_handler =& xoops_gethandler('module');
        $groups = is_object($xoopsUser) ? $xoopsUser->getGroups() : XOOPS_GROUP_ANONYMOUS;
        $moduleperm_handler =& xoops_gethandler('groupperm');
        $socialnet_module =& $module_handler->getByDirname(_MI_SOCIALNET_DIR);
        ...
        if ($itens) {
            ...
            foreach ($itens as $it) {
                ...
                if($first){
                    $ret[$it->getVar("mpb_10_order")][$it->getVar($it->id)] = "<li><a href='".$it->pegaLink()."' ".(($it->getVar("mpb_11_open") == 1) ? "target='_blank'" : "").">".$it->getVar("mpb_30_menu")."</a>";
                    $subpgs = ($it->getVar("mpb_12_exibesub")) ? $it->tem_subcategorias($it->getVar($it->id), true) : 0;
                    $ret[$it->getVar("mpb_10_order")][$it->getVar($it->id)] .= ($subpgs) ? "<ul>\n".$it->geraMenuCSS(new CriteriaCompo(new Criteria("mpb_10_idpai", $it->getVar($it->id))), false)."\n</ul>\n</li>\n" : "</li>\n";
                }else{
                    $ret .= "<li><a href='".$it->pegaLink()."' ".(($it->getVar("mpb_11_open") == 1) ? "target='_blank'" : "").">".$it->getVar("mpb_30_menu")."</a>";
                    $subpgs = ($it->getVar("mpb_12_exibesub")) ? $it->tem_subcategorias($it->getVar($it->id), true) : 0;
                    $ret .= ($subpgs) ? "<ul>\n".$it->geraMenuCSS(new CriteriaCompo(new Criteria("mpb_10_idpai", $it->getVar($it->id))), false)."\n</ul>\n</li>\n" : "</li>\n";
                }
            }
    ...
    function pegaLink($pg = null){
        if ($this->getVar('mpb_12_withoutlink')) {
            return 'javascript:void(0);';
        }elseif (substr($this->getVar("mpb_30_file"), 0, 4) == "ext:"){
            return substr($this->getVar("mpb_30_file"), 4);
        }
        $module_handler =& xoops_gethandler('module');
        $module =& $module_handler->getByDirname(_MI_SOCIALNET_DIR);
        $config_handler =& xoops_gethandler('config');
        $MeuModCFG =& $config_handler->getConfigsByCat(0,$module->getVar('mid'));
        if(!empty($MeuModCFG['socialnet_conf_names_id'])){
            if ($this->contar(new Criteria("mpb_30_menu", $this->getVar("mpb_30_menu"))) > 1) {
                return XOOPS_URL."/modules/socialnet/treemenu.php/$pg?tac=".$this->getVar($this->id);
            }else{
                return XOOPS_URL."/modules/socialnet/treemenu.php/$pg?tac=".urlencode(str_replace(" ", "_", $this->getVar("mpb_30_menu")));
            }
        }else{
            return XOOPS_URL."/modules/socialnet/treemenu.php/$pg?tac=".$this->getVar($this->id);
        }
    ...
    function updateCount(){
        $sql = 'UPDATE '.$this->tabela.' SET mpb_10_counter=mpb_10_counter+1 WHERE '.$this->id.'='.$this->getVar($this->id);
    ...
    function geraNavigation($id = null, $separador = "/", $style="style='font-weight:bold'"){
        if($id == null){
            ...
            if ($id > 0) {
                ...
                $new = new $classe($id);
                if ($new->getVar($new->id) > 0) {
                    $ret = "<a href='".$new->pegaLink()."' ".(($new->getVar("mpb_11_open") == 1) ? "target='_blank'" : "").">".$new->getVar("mpb_30_menu")."</a>";
                    if ($new->getVar("mpb_10_idpai") == 0) {
                        return "<a href='".XOOPS_URL."'>Home</a> $separador ".$ret;
                    }elseif ($new->getVar("mpb_10_idpai") > 0){
                        $ret = $new->geraNavigation($new->getVar("mpb_10_idpai"), $separador)." $separador ". $ret;
                    }else{
                        $module_handler =& xoops_gethandler('module');
                        $module = $module_handler->get($new->getVar("mpb_10_idpai")*-1);
                        $ret = "<a href='".XOOPS_URL."'>Home</a> $separador ".$module->mainLink()." $separador ".$ret;
                        return $ret;
                    }
                }
    ...
    function geraNavigationAdmin($id = null, $separador = "/", $style="style='font-weight:bold'"){
        $admin_url = XOOPS_URL."/modules/socialnet/admin/treemenu.php?op=list";
        if ($id == false) {
            return false;
        }elseif($id == null){
            $ret = "<span $style>".$this->getVar("mpb_30_menu")."</span>";
            if ($this->getVar("mpb_10_idpai") == 0) {
                $ret = "<a href='".$admin_url."&mpb_10_id=0'>"._AM_SOCIALNET_MENUP."</a> $separador ".$ret;
                return $ret;
            }elseif ($this->getVar("mpb_10_idpai") > 0){
                ...
                $module_handler =& xoops_gethandler('module');
                $module = $module_handler->get($this->getVar("mpb_10_idpai")*-1);
                $ret = "<a href='".$admin_url."&mpb_10_id=0'>"._AM_SOCIALNET_MENUP."</a> $separador <a href='".$admin_url."&mpb_10_id=".$this->getVar("mpb_10_idpai")."'>".$module->getVar('name')."</a> $separador ".$ret;
                return $ret;
            }
        }elseif ($id < 0){
            $module_handler =& xoops_gethandler('module');
            $module = $module_handler->get($id*-1);
            $ret = "<a href='".$admin_url."&mpb_10_id=0'>"._AM_SOCIALNET_MENUP."</a> $separador <a href='".$admin_url."&mpb_10_id=".$id."'>".$module->getVar('name')."</a>";
            return $ret;
        }else{
            if ($id > 0) {
                $classe = get_class($this);
                $new = new $classe($id);
                if ($new->getVar($new->id) > 0) {
                    $ret = "<a href='".$admin_url."&mpb_10_id=".$new->getVar("mpb_10_id")."'>".$new->getVar("mpb_30_menu")."</a>";
                    if ($new->getVar("mpb_10_idpai") == 0) {
                        return "<a href='".$admin_url."&mpb_10_id=0'>"._AM_SOCIALNET_MENUP."</a> $separador ".$ret;
                    }elseif ($new->getVar("mpb_10_idpai") > 0){
                        $ret = $new->geraNavigationAdmin($new->getVar("mpb_10_idpai"), $separador)." $separador ". $ret;
                    }else{
                        $module_handler =& xoops_gethandler('module');
                        $module = $module_handler->get($new->getVar("mpb_10_idpai")*-1);
                        $ret = "<a href='".$admin_url."&mpb_10_id=0'>"._AM_SOCIALNET_MENUP."</a> $separador <a href='".$admin_url."&mpb_10_id=".$new->getVar("mpb_10_idpai")."'>".$module->getVar('name')."</a> $separador ".$ret;
                        return $ret;
                    }
                }
            }else{
    ...
    function PegaSmileys($field = 'mpb_35_content')
    {
        ...
        if (empty($smileys)) {
            ...
            if ($result = $db->query('SELECT * FROM '.$db->prefix('smiles').' WHERE display=1')) {
                while ($smiles = $db->fetchArray($result)) {
                    $ret .= "<img onclick=\"tinyMCE.execInstanceCommand('$field', 'mceInsertContent', false, '<img src=\'".XOOPS_UPLOAD_URL."/".htmlspecialchars($smiles['smile_url'], ENT_QUOTES)."\' />');\" onmouseover='style.cursor=\"hand\"' src='".XOOPS_UPLOAD_URL."/".htmlspecialchars($smiles['smile_url'], ENT_QUOTES)."' alt='".$smiles['emotion']."' />";
                }
            }
        } else {
            $count = count($smiles);
            for ($i = 0; $i < $count; $i++) {
                if ($smiles[$i]['display'] == 1) {
                    $ret .= "<img onclick=\"tinyMCE.execInstanceCommand('$field', 'mceInsertContent', false, '<img src=\'".XOOPS_UPLOAD_URL."/".htmlspecialchars($smiles[$i]['smile_url'], ENT_QUOTES)."\' />');\" onmouseover='style.cursor=\"hand\"' src='".XOOPS_UPLOAD_URL."/".$myts->oopsHtmlSpecialChars($smiles[$i]['smile_url'])."' border='0' alt='".$smiles[$i]['emotion']."' />";
                }
            }
        }
        return $ret;
    ...
    function countSubs($mpb_10_id=null, $menu = false) {
        ...
        if(!$menu){
            ...
            $pages = $this->getPages();
            $subs_query = $this->db->query("SELECT ".$this->id." FROM " . $this->tabela . " where mpb_10_idpai =" . $id . " AND mpb_11_visible < 3");
