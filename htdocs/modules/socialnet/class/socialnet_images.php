...
/**
* socialnet_images class.  
* $this class is responsible for providing data access mechanisms to the data source 
* of XOOPS user class objects.
*/


class socialnet_images extends XoopsObject
{ 
	var $db;

// constructor
	function socialnet_images ($id=null)
	{
	...
	function load($id)
	{
		$sql = 'SELECT * FROM '.$this->db->prefix("socialnet_images").' WHERE cod_img='.$id;
	...
	function getAllsocialnet_imagess($criteria=array(), $asobject=false, $sort="cod_img", $order="ASC", $limit=0, $start=0)
	{
		...
		if ( !$asobject ) {
			$sql = "SELECT cod_img FROM ".$db->prefix("socialnet_images")."$where_query ORDER BY $sort $order";
			$result = $db->query($sql,$limit,$start);
			while ( $myrow = $db->fetchArray($result) ) {
				$ret[] = $myrow['socialnet_images_id'];
			}
		} else {
			$sql = "SELECT * FROM ".$db->prefix("socialnet_images")."$where_query ORDER BY $sort $order";
			$result = $db->query($sql,$limit,$start);
			while ( $myrow = $db->fetchArray($result) ) {
				$ret[] = new socialnet_images ($myrow);
			}
		}
		return $ret;
	}
...
// -------------------------------------------------------------------------
// ------------------socialnet_images user handler class -------------------
// -------------------------------------------------------------------------
/**
* socialnet_imageshandler class.  
* This class provides simple mecanisme for socialnet_images object and generate forms for inclusion etc
*/

class Xoopssocialnet_imagesHandler extends XoopsObjectHandler
{

	/**
	* create a new socialnet_images
	* 
	* @param bool $isNew flag the new objects as "new"?
	* @return object socialnet_images
	*/
	function &create($isNew = true)	{
		$socialnet_images = new socialnet_images();
		if ($isNew) {
			$socialnet_images->setNew();
		}
		else{
		$socialnet_images->unsetNew();
		}

		
		return $socialnet_images;
	}

	/**
	* retrieve a socialnet_images
	* 
	* @param int $id of the socialnet_images
	* @return mixed reference to the {@link socialnet_images} object, FALSE if failed
	*/
	function &get($id)	{
			$sql = 'SELECT * FROM '.$this->db->prefix('socialnet_images').' WHERE cod_img='.$id;
			...
			if ($numrows == 1) {
				$socialnet_images = new socialnet_images();
				$socialnet_images->assignVars($this->db->fetchArray($result));
				return $socialnet_images;
			}
				return false;
	}
...
/**
* insert a new socialnet_images in the database
* 
* @param object $socialnet_images reference to the {@link socialnet_images} object
* @param bool $force
* @return bool FALSE if failed, TRUE if already present and unchanged or successful
*/
	function insert(&$socialnet_images, $force = false) {
		Global $xoopsConfig;
		if (get_class($socialnet_images) != 'socialnet_images') {
				return false;
		}
		if (!$socialnet_images->isDirty()) {
				return true;
		}
		if (!$socialnet_images->cleanVars()) {
				return false;
		}
		foreach ($socialnet_images->cleanVars as $k => $v) {
				${$k} = $v;
		}
		$now = "date_add(now(), interval ".$xoopsConfig['server_TZ']." hour)";
		if ($socialnet_images->isNew()) {
			// ajout/modification d'un socialnet_images
			$socialnet_images = new socialnet_images();
			$format = "INSERT INTO %s (cod_img, title, data_creation, data_update, uid_owner, url, private)";
			$format .= "VALUES (%u, %s, %s, %s, %s, %s, 0)";
			$sql = sprintf($format , 
			$this->db->prefix('socialnet_images'), 
			$cod_img
			...
			$sql = sprintf($format, $this->db->prefix('socialnet_images'),
			$cod_img
			,$this->db->quoteString($title)
			,$now
			,$now
		...
		$socialnet_images->assignVar('cod_img', $cod_img);
		return true;
	}

	/**
	 * delete a socialnet_images from the database
	 * 
	 * @param object $socialnet_images reference to the socialnet_images to delete
	 * @param bool $force
	 * @return bool FALSE if failed.
	 */
	function delete(&$socialnet_images, $force = false)
	{
		if (get_class($socialnet_images) != 'socialnet_images') {
			return false;
		}
		$sql = sprintf("DELETE FROM %s WHERE cod_img = %u", $this->db->prefix("socialnet_images"), $socialnet_images->getVar('cod_img'));
		if (false != $force) {
			$result = $this->db->queryF($sql);
		} else {
			$result = $this->db->query($sql);
	...
	/**
	* retrieve socialnet_imagess from the database
	* 
	* @param object $criteria {@link CriteriaElement} conditions to be met
	* @param bool $id_as_key use the UID as key for the array?
	* @return array array of {@link socialnet_images} objects
	*/
	function &getObjects($criteria = null, $id_as_key = false)
	{
		$ret = array();
		$limit = $start = 0;
		$sql = 'SELECT * FROM '.$this->db->prefix('socialnet_images');
		...
		while ($myrow = $this->db->fetchArray($result)) {
			$socialnet_images = new socialnet_images();
			$socialnet_images->assignVars($myrow);
			if (!$id_as_key) {
				$ret[] =& $socialnet_images;
			} else {
				$ret[$myrow['cod_img']] =& $socialnet_images;
			}
			unset($socialnet_images);
		}
		return $ret;
	}

	/**
	* count socialnet_imagess matching a condition
	* 
	* @param object $criteria {@link CriteriaElement} to match
	* @return int count of socialnet_imagess
	*/
	function getCount($criteria = null)
	{
		$sql = 'SELECT COUNT(*) FROM '.$this->db->prefix('socialnet_images');
		if (isset($criteria) && is_subclass_of($criteria, 'criteriaelement')) {
			$sql .= ' '.$criteria->renderWhere();
		}
		$result = $this->db->query($sql);
	...
	/**
	* delete socialnet_imagess matching a set of conditions
	* 
	...
	*/
	...
	{
		$sql = 'DELETE FROM '.$this->db->prefix('socialnet_images');
		if (isset($criteria) && is_subclass_of($criteria, 'criteriaelement')) {
			$sql .= ' '.$criteria->renderWhere();
		}
		if (!$result = $this->db->query($sql)) {
	...
	*/
	function renderFormSubmit($maxbytes,$xoopsTpl)
	{
		...
		$form 				= new XoopsThemeForm(_MD_SOCIALNET_SUBMIT_PIC_TITLE, "form_picture", "submit_album.php", "post", true);
		$field_url 			= new XoopsFormFile(_MD_SOCIALNET_SELECT_PHOTO, "sel_photo", 2000000);
		$field_desc 			= new XoopsFormText(_MD_SOCIALNET_CAPTION, "caption", 35, 55);
		$form->setExtra('enctype="multipart/form-data"');
		$button_send 	= new XoopsFormButton("", "submit_button", _MD_SOCIALNET_UPLOADPICTURE, "submit");
	...
	*/
	function renderFormEdit($caption,$cod_img,$filename)
	{
		...
		$form 		= new XoopsThemeForm(_MD_SOCIALNET_EDITDESC, "form_picture", "editdesc.php", "post", true);
	...
	*/	
	...
	function getLastPictures($limit)
	{
		...
		$sql = 'SELECT uname, t.uid_owner, t.url FROM '.$this->db->prefix('socialnet_images').' AS t, '.$this->db->prefix('users');
		...
		function getLastPicturesForBlock($limit)
	{
		...
		$sql = 'SELECT uname, t.uid_owner, t.url, t.title FROM '.$this->db->prefix('socialnet_images').' AS t, '.$this->db->prefix('users');
