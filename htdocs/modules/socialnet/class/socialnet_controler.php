...
include_once("class/socialnet_suspensions.php");
if ( (str_replace('.', '', PHP_VERSION)) > 499 ){
  include_once("class/class.Id3v1.php");
} 


class SocialnetControler extends XoopsObject
{
	var $db;
	var $user;
	var $isOwner;
...
* @return void
*/
	function SocialnetControler($db,$user)
	{
		$this->db = $db;
		$this->user=$user;
		$this->isOwner=0;
...
*/
	 ...
	 */
      if ( !empty($_GET['uid'])){
        ...
        if (!(is_object($user))) {
          redirect_header("index.php",3,_MD_SOCIALNET_USERDOESNTEXIST);  
        }    
      }
	...
	function getNumbersSections(){
		...
		$nbSections['nbGroups']	= $this->relgroupusers_factory->getCount($criteriaGroups);
...
$this->album_factory 			= new Xoopssocialnet_imagesHandler($this->db);
$this->visitors_factory 		= new Xoopssocialnet_visitorsHandler($this->db);
$this->audio_factory           = new Xoopssocialnet_audioHandler($this->db); 
$this->videos_factory 			= new Xoopssocialnet_youtubeHandler($this->db);
$this->petitions_factory 		= new Xoopssocialnet_friendpetitionHandler($this->db);
$this->friendships_factory 		= new Xoopssocialnet_friendshipHandler($this->db);
$this->relgroupusers_factory 	= new Xoopssocialnet_relgroupuserHandler($this->db);
$this->scraps_factory 			= new Xoopssocialnet_scrapsHandler($this->db);
$this->groups_factory 	   		= new Xoopssocialnet_groupsHandler($this->db);
$this->configs_factory 	   		= new Xoopssocialnet_configsHandler($this->db);
$this->suspensions_factory 		= new Xoopssocialnet_suspensionsHandler($this->db);
...
class SocialnetVideoControler extends SocialnetControler {
	...
	function assignVideoContent($nbVideos, $videos)
	{
		if ($nbVideos==0)
		{
			...
			/**
     * Lets populate an array with the data from the videos
     */  
			$i = 0;
	...
	function checkPrivilege(){
		...
		if ($xoopsModuleConfig['enable_videos']==0){
			redirect_header("index.php?uid=".$this->owner->getVar('uid'),3,_MD_SOCIALNET_VIDEOSNOTENABLED);
		}
		$criteria = new Criteria('config_uid',$this->owner->getVar('uid'));
		if ($this->configs_factory->getCount($criteria)==1){
			$configs = $this->configs_factory->getObjects($criteria);
			...
			if (!$this->checkPrivilegeLevel($config)){
				redirect_header("index.php?uid=".$this->owner->getVar('uid'),10,_MD_SOCIALNET_NOPRIVILEGE);
			}
		}return true;
	}
}
...
class SocialnetControlerScraps extends SocialnetControler {
...
//		$fieldScrap = new XoopsFormTextArea('','text',_MD_SOCIALNET_ENTERTEXTSCRAP);
//		$fieldScrap->setExtra(' onclick="cleanScrapForm(text,\''._MD_SOCIALNET_ENTERTEXTSCRAP.'\')"');
//		
//		
//		$fieldScrabookUid = new XoopsFormHidden ("uid", $this->uidOwner);
//		
//		$submitButton = new XoopsFormButton("","post_scrap",_MD_SOCIALNET_SENDSCRAP,"submit");
//	
...
global $xoopsModuleConfig;
		if ($xoopsModuleConfig['enable_scraps']==0){
			redirect_header("index.php?uid=".$this->owner->getVar('uid'),3,_MD_SOCIALNET_SCRAPSNOTENABLED);
		}
...
class SocialnetControlerPhotos extends SocialnetControler {
	
	function checkPrivilege(){
global $xoopsModuleConfig;
		if ($xoopsModuleConfig['enable_pictures']==0){
			redirect_header("index.php?uid=".$this->owner->getVar('uid'),3,_MD_SOCIALNET_PICTURESNOTENABLED);
		}
		$criteria = new Criteria('config_uid',$this->owner->getVar('uid'));
		if ($this->configs_factory->getCount($criteria)==1){
			$configs = $this->configs_factory->getObjects($criteria);
...
class SocialnetAudioControler extends SocialnetControler {
          ...
          //audio info
            ...
            */  
            ...
            foreach ($audios as $audio){
                ...
                if ( (str_replace('.', '', PHP_VERSION)) > 499 ){  
                  $audio_path = XOOPS_ROOT_PATH.'/uploads/socialnet/mp3/'.$audio->getVar("url","s");
                  // echo $audio_path;
...
global $xoopsModuleConfig;
        if ($xoopsModuleConfig['enable_audio']==0){
            redirect_header("index.php?uid=".$this->owner->getVar('uid'),3,_MD_SOCIALNET_AUDIONOTENABLED);
        }
        $criteria = new Criteria('config_uid',$this->owner->getVar('uid'));
        if ($this->configs_factory->getCount($criteria)==1){
            $configs = $this->configs_factory->getObjects($criteria);
...
class SocialnetControlerFriends extends SocialnetControler  {
	
	function checkPrivilege(){
global $xoopsModuleConfig;
		if ($xoopsModuleConfig['enable_friends']==0){
			redirect_header("index.php?uid=".$this->owner->getVar('uid'),3,_MD_SOCIALNET_FRIENDSNOTENABLED);
		}
		$criteria = new Criteria('config_uid',$this->owner->getVar('uid'));
		if ($this->configs_factory->getCount($criteria)==1){
			$configs = $this->configs_factory->getObjects($criteria);
...
class SocialnetControlerGroups extends SocialnetControler  {
	
	function checkPrivilege(){
global $xoopsModuleConfig;
		if ($xoopsModuleConfig['enable_groups']==0){
			redirect_header("index.php?uid=".$this->owner->getVar('uid'),3,_MD_SOCIALNET_GROUPSNOTENABLED);
		}
		$criteria = new Criteria('config_uid',$this->owner->getVar('uid'));
		if ($this->configs_factory->getCount($criteria)==1){
			$configs = $this->configs_factory->getObjects($criteria);
...
class SocialnetControlerIndex extends SocialnetControler  {
...
class SocialnetControlerConfigs extends SocialnetControler{
	function checkPrivilege(){
		return true;
	}
}
