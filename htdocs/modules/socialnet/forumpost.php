...
if ( empty($forum) ) {
	redirect_header('forumstart.php', 4, _MD_SOCIALNET_FORUM_ERRORFORUM);
	exit();
} else {
	...
	if ( !$result = $xoopsDB->query($sql) ) {
		redirect_header('forumstart.php', 3,_MD_SOCIALNET_FORUM_ERROROCCURED);
		exit();
	}
	...
	// To get here, we have a logged-in user. So, check whether that user is allowed to view this private forum.
		...
		if ( $accesserror == 1 ) {
			redirect_header("forumview.php?order=".$order."&viewmode=".$viewmode."&forum=".$forum, 4,_MD_SOCIALNET_FORUM_NORIGHTTOPOST);
			exit();
		}
	} else {
		$accesserror = 0;
	...
	if ( !empty($_POST['contents_preview']) ) {
		...
		if ( !empty($post_id) ) {
			...
			if ( $editerror == 1 ) {
				redirect_header("forumviewtopic.php?topic_id=".$topic_id."&post_id=".$post_id."&order=".$order."&viewmode=".$viewmode."&pid=".$pid."&forum=".$forum,2,_MD_SOCIALNET_FORUM_EDITNOTALLOWED);
				exit();
			}
			$editor = $xoopsUser->getVar("uname");
   			$on_date .= _MD_SOCIALNET_FORUM_ON." ".formatTimestamp(time());
			//$message .= "\n\n<small>[ "._MD_SOCIALNET_FORUM_EDITEDBY." ".$editor." ".$on_date." ]</small>";
		} else {
			$isreply = 0;
			$isnew = 1;
			if ( is_object($xoopsUser) && empty($_POST['noname']) ) {
				...
				if ( $forumdata['forum_access'] == 2 ) {
					...
					if ( !empty($topic_id) ) {
						redirect_header("forumviewtopic.php?topic_id=".$topic_id."&order=".$order."&viewmode=".$viewmode."&pid=".$pid."&forum=".$forum,2,_MD_SOCIALNET_FORUM_ANONNOTALLOWED);
					} else {
						redirect_header("forumview.php?forum=".$forum,2,_MD_SOCIALNET_FORUM_ANONNOTALLOWED);
					}
					exit();
				}
			}
		...
		if($forumdata['allow_upload'] == 1) {
				...
				if(xoops_trim($fldname != '')) {
					...
					if ($uploader->fetchMedia($_POST['xoops_upload_file'][0])) {
						if ($uploader->upload()) {
							...
							if(!$sfiles->store()) {
								echo _MD_SOCIALNET_FORUM_UPLOAD_DBERROR_SAVE;
							}
						} else {
							echo _MD_SOCIALNET_FORUM_UPLOAD_ERROR. ' ' . $uploader->getErrors();
						}
					} else {
			...
			if(isset($_POST['delattach'])) {
				...
				$uid = -1;
				$sqluid = 'SELECT uid FROM '.$xoopsDB->prefix('socialnet_forumposts').' WHERE post_id='.$post_id;
		...
		$tags['THREAD_URL'] = XOOPS_URL . '/modules/' . $xoopsModule->dirname() . '/forumviewtopic.php?forum=' . $forum . '&post_id='.$postid.'&topic_id=' . $forumpost->topic();
		$tags['POST_URL'] = $tags['THREAD_URL'] . '#forumpost' . $postid;
		include_once XOOPS_ROOT_PATH.'/modules/socialnet/include/socialnet_forumnotification.inc.php';
		$forum_info = socialnet_notify_iteminfo ('forum', $forum);
		...
		if ( $_POST['viewmode'] == "flat" ) {
			redirect_header("forumviewtopic.php?topic_id=".$forumpost->topic()."&amp;post_id=".$postid."&amp;order=".$order."&amp;viewmode=flat&amp;pid=".$pid."&amp;forum=".$forum."#forumpost".$postid."",2,_MD_SOCIALNET_FORUM_THANKSSUBMIT);
			exit();
		} else {
			$post_id = $forumpost->postid();
			redirect_header("forumviewtopic.php?topic_id=".$forumpost->topic()."&amp;post_id=".$postid."&amp;order=".$order."&amp;viewmode=thread&amp;pid=".$pid."&amp;forum=".$forum."#forumpost".$postid."",2,_MD_SOCIALNET_FORUM_THANKSSUBMIT);
			exit();
		}
	}
