...
//If no access
if (!$gperm_handler->checkRight("socialnet_submit", $perm_itemid, $groups, $module_id)) {
...
$approveprivilege = 0;
if ($xoopsUser && $gperm_handler->checkRight("socialnet_approve", $perm_itemid, $groups, $module_id)) {
    $approveprivilege = 1;
}
...
}elseif ( (isset($_REQUEST['op']) ) && (isset($_REQUEST['storyid'])))
{
	...
	{
        $op = 'delete';
    }elseif ($approveprivilege && (($_REQUEST['op'] == _MD_SOCIAL_NW_OVERRIDE) ))
	{
		$op = _MD_SOCIAL_NW_OVERRIDE;
    }elseif ($approveprivilege && (($_REQUEST['op'] == _MD_SOCIAL_NW_FINDVERSION) ))
	{
		$op = _MD_SOCIAL_NW_FINDVERSION;
    }elseif ($approveprivilege && (($_REQUEST['op'] == 'override_ok') ))
	{
		$op = 'override_ok';
    }else
	{
        redirect_header("news.php", 0, _NOPERM);
        exit();
    }
	if (isset($_REQUEST['storyid']))
	{
...
switch ($op) {
    case "delete":
        ...
        else
        {
            ...
            xoops_confirm( array( 'op' => 'delete', 'storyid' => $_REQUEST['storyid'], 'ok' => 1 ), 'submit_news.php', _AM_SOCIALNET_RUSUREDEL );
        }
        break;
    ...
    case 'edit':
        ...
        echo "<h4>" . _AM_SOCIALNET_EDITARTICLE . "</h4>";
...
case "preview":
	...
	// Replace deprecated themecenterposts function
...
//	}else
	...
	$pform = new XoopsThemeForm($p_title, "previewform", XOOPS_URL.'/modules/socialnet/submit_news.php');
	$pform->display();
	print "$p_hometext";
...
case "post":
	...
	if ( empty( $_POST['storyid'] ) ) {
	    ...
	    $story = new SocialnetStory( $_POST['storyid'] );
	...
	if ($result) {
	    ...
	    $tags['STORY_URL'] = XOOPS_URL . '/modules/' . $xoopsModule->getVar('dirname') . '/newsarticle.php?storyid=' . $story->storyid();
	    if ( $approve == 1 && $oldapprove == 0 && $story->published <= time()) {
	        $notification_handler->triggerEvent('global', 0, 'new_story', $tags);
	    } elseif ($approve != 1) {
	        $tags['WAITINGSTORIES_URL'] = XOOPS_URL . '/modules/' . $xoopsModule->getVar('dirname') . '/submit_news.php?op=edit&amp;storyid='.$story->storyid();
	    ...
	    if(isset($_POST['xoops_upload_file'])&& isset($_FILES[$_POST['xoops_upload_file'][0]]))
	    {
	        ...
	        if(trim($fldname!=''))
	        {
	            ...
	            // Actually : Web pictures (png, gif, jpeg), zip, doc, xls, pdf, gtar, tar, txt, tiff, htm, xml, ico,swf flv, mp3, bmp, ra, mov, swc. swf not allow by xoops, not SOCIAL
	            ...
	            if ($uploader->fetchMedia($_POST['xoops_upload_file'][0]))
	            {
	                if ($uploader->upload())
	                {
	                    ...
	                    $sfiles->setMimetype($sfiles->giveMimetype(XOOPS_UPLOAD_PATH.'/socialnet/news'.$uploader->getMediaName()));
	                    $sfiles->setDownloadname($destname);
	                    if(!$sfiles->store())
	                    {
	                        //echo _AM_SOCIALNET_UPLOAD_DBERROR_SAVE;
	...
	if (!isset($message)) {
	    $message = _MD_SOCIAL_NW_THANKS;
	}
	redirect_header("news.php",2, $message);
	break;
	
	case _MD_SOCIAL_NW_OVERRIDE:
	   ...
	   $message .= _MD_SOCIAL_NW_TRYINGTOSAVE." ".$story->version.".".$story->revision.".".$story->revisionminor." <br />";
	   $higher_versions = $story->getVersions(true);
	   if (count($higher_versions) > 0) {
	       $message .= sprintf(_MD_SOCIAL_NW_VERSIONSEXIST, count($higher_versions));
	       $message .= "<br />";
	       foreach ($higher_versions as $key => $version) {
	           $message .= $version['version'].".".$version['revision'].".".$version['revisionminor']."<br />";
	       }
	   }
	   $message .= _MD_SOCIAL_NW_AREYOUSUREOVERRIDE;
	   xoops_confirm($hiddens, 'submit_news.php', $message, _YES);
	   break;
	...
	case 'override_ok':
	   ...
	   if ($story->overrideVersion()) {
	       $message = sprintf(_MD_SOCIAL_NW_VERSIONUPDATED, $story->version.".".$story->revision.".".$story->revisionminor);
	   }
	   else {
	       $message = $story->renderErrors();
	   }
	...
	case _MD_SOCIAL_NW_FINDVERSION:
	   if (!$approveprivilege || !$xoopsUser) {
	       redirect_header('news.php', 3, _NOPERM);
	       exit();
	   }
