...
if(!($GLOBALS['xoopsSecurity']->check())) {redirect_header($_SERVER['HTTP_REFERER'], 5, _MD_SOCIALNET_TOKENEXPIRED);}

$uid = intval($_POST['uid']);
/**
* Creating the factory  loading the picture changing its caption
*/
$suspensions_factory = new Xoopssocialnet_suspensionsHandler($xoopsDB);
...
if($xoopsUser->isAdmin(1))
{
	...
	$thisUser->setVar('user_sig',sprintf(_MD_SOCIALNET_SUSPENDED,formatTimestamp( time()+intval($_POST['time']),'m')));
	$member_handler->insertUser($thisUser);
	redirect_header('index.php?uid='.$uid,300,_MD_SOCIALNET_USERSUSPENDED);
}
