...
if($friendpetition_factory->getCount($criteria)>0)
{
	if(($friendship_level>0) && ($petition = $friendpetition_factory->getObjects($criteria)))
	{
		...
		redirect_header(XOOPS_URL.'/modules/socialnet/friends.php?uid='.$friend2_uid,3,_MD_SOCIALNET_FRIENDMADE);
	}
	else
	{
		...
		{
			$friendpetition_factory->deleteAll($criteria);
			redirect_header(XOOPS_URL.'/modules/socialnet/youtube.php?uid='.$uid,3,_MD_SOCIALNET_FRIENDSHIPNOTACCEPTED);
		}
		redirect_header(XOOPS_URL.'/modules/socialnet/index.php?uid='.$uid,3,_MD_SOCIALNET_ANERROR);
	}
}
else {redirect_header(XOOPS_URL.'/modules/socialnet/index.php?uid='.$uid,3,_MD_SOCIALNET_ANERROR);}
