...
function socialnet_related_exibe($options){
			...
			if ($all_related) {
				foreach ($all_related as $k=>$v) {
					if(strlen($v->getVar($options[2])) > $options[3]){
						$title = substr($v->getVar($options[2]),0, $options[3])."...";
					}else{
						$title = $v->getVar($options[2]);
					}
					$block['relpages'][$k]['title'] = $title;
...
function socialnet_related_edita($options){
...
PICKER;
	...
	$form .= _MB_SOCIALNET_BLO_SHOW." <select name='options[2]'>
	<option value='mpb_30_menu'";
	$form .= ($options[2]==('mpb_30_menu'))? 'selected="selected"':' ';
	$form .= ">"._MB_SOCIALNET_BLO_MPB_30_MENU."</option>
	<option value='mpb_30_title'";
	$form .= ($options[2]==('mpb_30_title'))? 'selected="selected"':' ';
	$form .= ">"._MB_SOCIALNET_BLO_MPB_30_TITLE."</option></select><br />";
	$form .= _MB_SOCIALNET_BLO_OPT_CARACTS." <input type='text' name='options[3]' value='".$options[3]."' />";
	return $form;
}
  ?>