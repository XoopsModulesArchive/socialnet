...
function b_socialnet_random_show($options)	// 10=Items count, 30=Title's length
{
	$block = array();
	$start = 0;
	...
	$pages = array();
	$pages = $socialnet_handler->getRandomPages($criteria, false, $limit);
 ...
 * The edit function
 */
function b_socialnet_random_edit($options)	// 10=Items count, 30=Title's length
{
	$form= '';
...
* Block, "on the fly".
*/
function b_socialnet_random_onthefly($options)
{
	$options = explode('|',$options);
	$block = & b_socialnet_random_show($options);

	$tpl = new XoopsTpl();
	$tpl->assign('block', $block);
	$tpl->display('db:socialnet_block_random.html');
}
?>