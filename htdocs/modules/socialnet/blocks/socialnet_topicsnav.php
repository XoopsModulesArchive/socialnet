...
function b_socialnet_topicsnav_show($options) {
    include_once(XOOPS_ROOT_PATH."/modules/socialnet/class/class.newstopic.php");
	global $xoopsDB, $xoopsUser;
	$block = array();
	$topics = SocialnetTopic::getAllTopics($options[0]);
...
function b_socialnet_topicsnav_edit($options) {
    include_once (XOOPS_ROOT_PATH."/class/xoopsformloader.php");
    $form = new XoopsFormElementTray('', '<br/>');
    $restrict_select = new XoopsFormRadioYN(_MB_SOCIALNET_BLO_NEWS_RESTRICTTOPICS, 'options[0]', $options[0]);
