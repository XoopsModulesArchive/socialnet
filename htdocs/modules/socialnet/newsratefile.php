...
if ($storyid > 0) {
    ...
    redirect_header('news.php', 3, _MD_SOCIAL_NW_NOSTORY);
    exit();
}
if($_POST['submit'] != '' && $storyid > 0)
{
    if ($article->rateStory($_POST['rating'])) {
        $ratemessage = _MD_SOCIAL_NW_RATING_SUCCESSFUL;
    }
    else {
        $ratemessage = $article->renderErrors();
    }
	redirect_header("newsarticle.php?storyid=".$article->storyid(), 3 ,$ratemessage);
	exit();

} 
else 
{
    $xoopsOption['template_main'] = "socialnet_ratearticle.html";
	include XOOPS_ROOT_PATH."/header.php";
	include ('include/ratingform.inc.php');
}
$xoopsTpl->assign('breadcrumb', $article->getPath(true)." > "._MD_SOCIAL_NW_RATE);
