...
if (!defined("XOOPS_CONF_USER")) {
  ...
  $mod_socialnet  =& $mod_handler->getByDirname('profile');
  if ($mod_socialnet->getVar('isactive')==1) {
     define("XOOPS_CONF_USER",0);
	 $xoopsConfigUser =& $config_handler->getConfigsByCat(0,$mod_socialnet->getVar('mid'));
	 unset($mod_handler);
	 unset($mod_socialnet);
  } else {
