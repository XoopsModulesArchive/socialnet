...
$sql = 'SELECT forum_type, forum_name, forum_access, allow_html, allow_sig, posts_per_page, hot_threshold, topics_per_page FROM '.$xoopsDB->prefix('socialnet_forums').' WHERE forum_id = '.$forum;
if ( !$result = $xoopsDB->query($sql) ) {
	redirect_header("forumstart.php", 2, _MD_SOCIALNET_FORUM_ERRORCONNECT);
	exit();
}
if ( !$forumdata = $xoopsDB->fetchArray($result) ) {
	redirect_header("forumstart.php", 2, _MD_SOCIALNET_FORUM_ERROREXIST);
	exit();
}
// this page uses smarty template
// this must be set before including main header.php
$xoopsOption['template_main'] = 'socialnet_forumview.html';
...
if ( $forumdata['forum_type'] == 1 ) {
	...
	if ( $accesserror == 1 ) {
		redirect_header("forumstart.php",2,_MD_SOCIALNET_FORUM_NORIGHTTOACCESS);
		exit();
	}
	$can_post = 1;
	$show_reg = 1;
...
if ( $can_post == 1 ) {
	$xoopsTpl->assign('viewer_can_post', true);
  	$xoopsTpl->assign('forum_post_or_register', "<a href=\"forumnewtopic.php?forum=".$forum."\"><img src=\"".$bbImage['post']."\" alt=\""._MD_SOCIALNET_FORUM_POSTNEW."\" /></a>");
} else {
...
$xoopsTpl->assign('lang_moderatedby', _MD_SOCIALNET_FORUM_MODERATEDBY);
...
$sel_sort_array = array("t.topic_title"=>_MD_SOCIALNET_FORUM_TOPICTITLE, "t.topic_replies"=>_MD_SOCIALNET_FORUM_NUMBERREPLIES, "u.uname"=>_MD_SOCIALNET_FORUM_TOPICPOSTER, "t.topic_views"=>_MD_SOCIALNET_FORUM_VIEWS, "p.post_time"=>_MD_SOCIALNET_FORUM_LASTPOSTTIME);
...
$xoopsTpl->assign('lang_sortby', _MD_SOCIALNET_FORUM_SORTEDBY);
...
$forum_selection_order .= '<option value="ASC"'.(($sortorder == "ASC") ? ' selected="selected"' : '').'>'._MD_SOCIALNET_FORUM_ASCENDING.'</option>';
$forum_selection_order .= '<option value="DESC"'.(($sortorder == "DESC") ? ' selected="selected"' : '').'>'._MD_SOCIALNET_FORUM_DESCENDING.'</option>';
...
foreach ($sel_since_array as $sort_since_v) {
	$forum_selection_since .= '<option value="'.$sort_since_v.'"'.(($sortsince == $sort_since_v) ? ' selected="selected"' : '').'>'.sprintf(_MD_SOCIALNET_FORUM_FROMLASTDAYS,$sort_since_v).'</option>';
}
$forum_selection_since .= '<option value="365"'.(($sortsince == 365) ? ' selected="selected"' : '').'>'.sprintf(_MD_SOCIALNET_FORUM_THELASTYEAR,365).'</option>';
$forum_selection_since .= '<option value="1000"'.(($sortsince == 1000) ? ' selected="selected"' : '').'>'.sprintf(_MD_SOCIALNET_FORUM_BEGINNING,1000).'</option>';
$forum_selection_since .= '</select>';

// assign to template
$xoopsTpl->assign('forum_selection_since', $forum_selection_since);
$xoopsTpl->assign('lang_go', _MD_SOCIALNET_FORUM_GO);

$xoopsTpl->assign('h_topic_link', "forumview.php?forum=$forum&amp;sortname=t.topic_title&amp;sortsince=$sortsince&amp;sortorder=". (($sortname == "t.topic_title" && $sortorder == "DESC") ? "ASC" : "DESC"));
$xoopsTpl->assign('lang_topic', _MD_SOCIALNET_FORUM_TOPIC);

$xoopsTpl->assign('h_reply_link', "forumview.php?forum=$forum&amp;sortname=t.topic_replies&amp;sortsince=$sortsince&amp;sortorder=". (($sortname == "t.topic_replies" && $sortorder == "DESC") ? "ASC" : "DESC"));
$xoopsTpl->assign('lang_replies', _MD_SOCIALNET_FORUM_REPLIES);

$xoopsTpl->assign('h_poster_link', "forumview.php?forum=$forum&amp;sortname=u.uname&amp;sortsince=$sortsince&amp;sortorder=". (($sortname == "u.uname" && $sortorder == "DESC") ? "ASC" : "DESC"));
$xoopsTpl->assign('lang_poster', _MD_SOCIALNET_FORUM_POSTER);

$xoopsTpl->assign('h_views_link', "forumview.php?forum=$forum&amp;sortname=t.topic_views&amp;sortsince=$sortsince&amp;sortorder=". (($sortname == "t.topic_views" && $sortorder == "DESC") ? "ASC" : "DESC"));
$xoopsTpl->assign('lang_views', _MD_SOCIALNET_FORUM_VIEWS);

$xoopsTpl->assign('h_date_link', "forumview.php?forum=$forum&amp;sortname=p.post_time&amp;sortsince=$sortsince&amp;sortorder=". (($sortname == "p.post_time" && $sortorder == "DESC") ? "ASC" : "DESC"));
...
if(get_show_name($forum)) {
	$sql = 'SELECT t.*, u.uname, u.name, u2.name as last_poster, u2.uname as last_poster2, p.post_time as last_post_time, p.icon FROM '.$xoopsDB->prefix("socialnet_forumtopics").' t LEFT JOIN '.$xoopsDB->prefix('users').' u ON u.uid = t.topic_poster LEFT JOIN '.$xoopsDB->prefix('socialnet_forumposts').' p ON p.post_id = t.topic_last_post_id LEFT JOIN '.$xoopsDB->prefix('users').' u2 ON  u2.uid = p.uid WHERE t.forum_id = '.$forum.' AND (p.post_time > '.$startdate.' OR t.topic_sticky=1) ORDER BY topic_sticky DESC, '.$sortname.' '.$sortorder;
} else {
	$sql = 'SELECT t.*, u.uname, u.name, u2.uname as last_poster, p.post_time as last_post_time, p.icon FROM '.$xoopsDB->prefix("socialnet_forumtopics").' t LEFT JOIN '.$xoopsDB->prefix('users').' u ON u.uid = t.topic_poster LEFT JOIN '.$xoopsDB->prefix('socialnet_forumposts').' p ON p.post_id = t.topic_last_post_id LEFT JOIN '.$xoopsDB->prefix('users').' u2 ON  u2.uid = p.uid WHERE t.forum_id = '.$forum.' AND (p.post_time > '.$startdate.' OR t.topic_sticky=1) ORDER BY topic_sticky DESC, '.$sortname.' '.$sortorder;
}
...
while ( $myrow = $xoopsDB->fetchArray($result) ) {
	...
	$pagination = '';
	$addlink = '';
	$topiclink = 'forumviewtopic.php?topic_id='.$myrow['topic_id'].'&amp;forum='.$forum;
...
$xoopsTpl->assign('lang_by', _MD_SOCIALNET_FORUM_BY);
...
$xoopsTpl->assign('lang_newposts', _MD_SOCIALNET_FORUM_NEWPOSTS);
$xoopsTpl->assign('lang_hotnewposts', _MD_SOCIALNET_FORUM_MORETHAN);
$xoopsTpl->assign('lang_hotnonewposts', _MD_SOCIALNET_FORUM_MORETHAN2);
$xoopsTpl->assign('lang_nonewposts', _MD_SOCIALNET_FORUM_NONEWPOSTS);
$xoopsTpl->assign('lang_legend', _MD_SOCIALNET_FORUM_LEGEND);
$xoopsTpl->assign('lang_topiclocked', _MD_SOCIALNET_FORUM_TOPICLOCKED);
$xoopsTpl->assign('lang_topicsticky', _MD_SOCIALNET_FORUM_TOPICSTICKY);
$xoopsTpl->assign("lang_search", _MD_SOCIALNET_FORUM_SEARCH);
$xoopsTpl->assign("lang_advsearch", _MD_SOCIALNET_FORUM_ADVSEARCH);

$sql = 'SELECT COUNT(*) FROM '.$xoopsDB->prefix('socialnet_forumtopics').' WHERE forum_id = '.$forum.' AND (topic_time > '.$startdate.' OR topic_sticky = 1)';
...
// CHECK THIS PROBLEM IN LINE 304, 304, 307
//include XOOPS_ROOT_PATH.'/modules/socialnet/include/forumfunctions.php';
//$meta_keywords = socialnet_createmeta_keywords($myts->displayTarea($keywords));
