...
die("XOOPS root path not defined");
	...
	*/
function socialnet_search($queryarray, $andor, $limit, $offset, $userid)
{
	global $xoopsDB, $module;
	//getting the url to the uploads directory
	$module_handler =& xoops_gethandler('module');
	$modulo         =& $module_handler->getByDirname('socialnet');
	$config_handler =& xoops_gethandler('config');
	$moduleConfig   =& $config_handler->getConfigsByCat(0, $modulo->getVar('mid'));
	$path_uploadimages = XOOPS_UPLOAD_URL;

	$ret = array();
	$sql = "SELECT cod_img,	title, 	data_creation, 	uid_owner, url FROM ".$xoopsDB->prefix("socialnet_images")." WHERE ";
	if ( $userid != 0 ) {
	...
	$i = 0;
	$stringofimage = 'images/profile/search.png" />';
 	while ( $myrow = $xoopsDB->fetchArray($result) ) {
		if ( $userid != 0 ) {
		if ($limit>5){
		$ret[$i]['image'] = "images/profile/search.png' /><a href='".XOOPS_URL."/modules/socialnet/album.php?uid=".$myrow['uid_owner']."'><img src='".$path_uploadimages."/thumb_".$myrow['url']."' /></a><br />"."<img src=".XOOPS_URL."/modules/socialnet/images/profile/search.png" ;		
		...
		$stringofimage .= '<a href="'.XOOPS_URL."/modules/socialnet/album.php?uid=".$myrow['uid_owner'].'" title="'.$myrow['title'].'"><img src="'.$path_uploadimages.'/thumb_'.$myrow['url'].'" /></a>&nbsp;' ;
	...
	if ( $userid != 0 && $i>0) {
		...
		$stringofimage .= '<img src="'.XOOPS_URL.'/modules/socialnet/images/profile/search.png';
