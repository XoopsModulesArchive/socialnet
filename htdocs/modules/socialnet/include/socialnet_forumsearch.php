...
function socialnet_forumsearch($queryarray, $andor, $limit, $offset, $userid){
	...
	$sql = "SELECT p.post_id,p.topic_id,p.forum_id,p.post_time,p.uid,p.subject FROM ".$xoopsDB->prefix("socialnet_forumposts")." p LEFT JOIN ".$xoopsDB->prefix("socialnet_forumposts_text")." t ON t.post_id=p.post_id LEFT JOIN ".$xoopsDB->prefix("socialnet_forums")." f ON f.forum_id=p.forum_id  $where";
	if ( $userid != 0 ) {
	...
	$i = 0;
 	while($myrow = $xoopsDB->fetchArray($result)){
		$ret[$i]['link'] = "forumviewtopic.php?topic_id=".$myrow['topic_id']."&amp;forum=".$myrow['forum_id'].$searchparam."#forumpost".$myrow['post_id'];
